...
bool LinkedList::removeElement(string someClubName, string someUnivName)
 {
   ...
   if(pos == NULL)
       ...
       else
	 {
	   ...
	   while(pos != NULL && pos->clubName.compare(someClubName) != 0 && pos->univName.compare(someUnivName) != 0)
	     {
	       nextNode = pos; 
	       pos = pos->next;
	       ...
	       //removes if matches
	       /*
	       if(pos->clubName.compare(someClubName) == 0 && pos->univName.compare(someUnivName) == 0)
		 {
	   ...
	   if(pos != NULL)
	     {
	       if(pos->clubName.compare(head->clubName) == 0 && pos->univName.compare(head->univName) == 0)
		 {
		   head = pos->next; 
		   return true; 
		 }
