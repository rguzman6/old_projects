...
bool LinkedList::insertEdge(int vertex, int weight)
{
  ...
  if(head == NULL || add->vertex <= head->vertex)
      ...
      if(temp->vertex == add->vertex)
	{
	  //insert by weight
	  /*
	  while(temp != NULL && temp->weight < add->weight && temp->vertex == add->vertex)
	  
	  while(temp->next != NULL && temp->next->weight <= add->weight && temp->next->vertex == add->vertex)
	    {
	      trail = temp; 
	  ...
	  */
	  ...
	  if(temp->vertex == add->vertex && temp->weight == add->weight)
	    {
	      return false; 
	    }
