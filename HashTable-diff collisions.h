...
void HashTable::insertElement(string id, int credits, int capacity, string days, string time)
{
  ...
  int key = 0; 
  //keep track of number of collisions the element has before being inserted
  int collisions = 0; 
  ...
  do
    {
      ...
      if(table[j].courseId.compare("none") == 0)
	 {
      ...
      //if a spot doesn't have a default value, increase the number of collisions to keep track of what element trying to enter
      else
	 {
	  collisions++; 
	 }
