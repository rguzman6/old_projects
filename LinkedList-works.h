...
bool LinkedList::removeElement(string someClubName, string someUnivName)
 {
   ...
   //else
	 ...
	 //{
	   ...
	   //makes sure that not at the end of the linked list
	   while(pos->next != NULL)
	     {
	       //checks if pointer to the next position matches the element to remove
	       //removes if matches
	       if((pos->next->clubName.compare(someClubName) == 0) && (pos->next->univName.compare(someUnivName) == 0))
		 {
		//matches link current position to two positions over, "ignoring" (removing) the current node
		   pos->next = pos->next->next; 
		   return true; //successful deletion
		 }
	       //if the current position doesn't match the element to remove, go to next element in the linked list to continue to loop
	       else
		 {
		   pos = pos->next; 
		 }
