...
void Parser::secondErrorCheck()
{
  ...
  /*cout << "poly variables" << endl; 
  for(int r = 0; r < polyVariables.size(); r++)
    {
      cout << polyVariables[r] << " " << endl; 
    }
    cout << endl;*/ 
  bool error2 = false; 
  ...
  for(int i = 0; i < blocks.size(); i++)
    {
      ...
      int blockNum; 
      int numVariables = polyVariables.size(); 
      //cout << "new block" << endl; 

      for(int j = 0; j < singleBlock.size(); j++)
	{
	  //cout << "poly: " << polyVariables[j] << endl; 
	  for(int k = 0; k < polyVariables.size(); k++)
	    {
	      //cout << "eval block: " << singleBlock[k].key << endl; 
	      if(polyVariables[k] == singleBlock[j].key)
		{
		  check.push_back(singleBlock[j]);
		}
	    }
	  blockNum = singleBlock[j].evalLineNum; 
	}
      ...
      if(check.size() < polyVariables.size())
	{
	  ...
	  //cout << "# missing: " << numVariablesMissing << endl;
	  //blockNum = singleBlock[i].evalLineNum; 
  ...
  for(int i = 0; i < blocks.size(); i++)
    {
      ...
      for(int j = 0; j < singleBlock.size(); j++)
	{
	  cout << "key: " << singleBlock[j].key << " value: " << singleBlock[j].value << endl; 
	}
      */ 
...
Node* Parser::parseExpression()
{
  Token token; 
  Node* node = new Node();
  //cout << "parseExpressions" << endl; 
  node = parseTerm();
  //cout << "first parseTerm"  << endl; 
  token = lexer.peek(1);

  if(token.token_type == PLUS)
    {
      //cout << "parseExp if statement" << endl;
      Node* node2 = new Node();
      node2->left = node;
      //cout << "node2 left to first parseTerm"  << endl; 
      node2->data = "+";
      node2->type = "op";
      //cout << node2->data << endl; 
      expect(PLUS);
      node2->right = parseExpression();
      //cout << "node to right to parseExpression" << endl; 
      return node2; 
    }
...
Node* Parser::parseTerm()
{
  ...
  if(token.token_type == MULT)
    {
      //cout << "if statement parseTerm " << endl; 
      Node* node2 = new Node();
      node2->left = node;
      //cout << "node 2 left to first parseTerm"  << endl;  
      node2->data = "*";
      node2->type = "op";
      //cout << node2->data << endl; 
      expect(MULT);
      node2->right = parseTerm();
      //cout << "node2 to parseTerm"  << endl; 
      return node2; 
    }
  ...
  if(token.token_type == ID)
    {
      ...
      //cout << "nose to ID" << endl; 
      expect(ID);
      token = lexer.peek(1); 
      if(token.token_type == POWER)
	{
	  //cout << "parseMon with exp" << endl; 
	  Node* node2 = new Node();
	  node2 = parseExponent();
	  //cout << "set node2 to parseExponent"  << endl; 
	  node2->left = node;
	  //cout << "set left to ID "  << endl; 
	  return node2; 
	}
   }
...
Block Parser::parseInput()
{
  ...
  //cout << "key: " << sect.key << " line num: " << sect.lineNum << endl; 
  expect(ID);
  expect(EQUAL);
